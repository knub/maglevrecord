...
class SuperclassMismatchMigrationStringTest < TempDirTest
  ...
  def setup
    fp = write_to_file('class X < SuperclassOfX;include MaglevRecord::Base;def h;end;end;
                        class Y;include MaglevRecord::Base;attr_accessor :a;end')
    s = MaglevRecord::Snapshot.with_files[fp]
    write_to_file('class X < NotSuperclassOfX;include MaglevRecord::Base;def h;end;end
  ...
  def test_the_state_of_all_classes_is_restored
    ...
    assert_include? X.methods(false), 'h'
  end
  ...
  def test_superclass_mismatch_migration_string
    assert_equal "X.remove_superclass", superclass_mismatch_change.migration_string
  end
